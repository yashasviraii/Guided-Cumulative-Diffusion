...
class NodeLevelGNN(torch.nn.Module):
    """3-layer GCN predicting a per-node priority score in [0, 1] (pre-clip)."""

    def __init__(self, in_channels: int, hidden_channels: int = 64, num_layers: int = 3) -> None:
        super().__init__()
    ...
    def forward(self, x: torch.Tensor, edge_index: torch.Tensor, batch=None) -> torch.Tensor:
        """Return node-level predictions with shape ``[num_nodes]``."""
...
class SimpleGCNInference:
    ...
    def _try_load_gcn(self, state_dict: dict) -> bool:
        conv_idxs = {
            int(m.group(1))
            for k in state_dict
            if (m := re.match(r"convs\.(\d+)\.", k))
        }
        num_layers = max(conv_idxs) + 1 if conv_idxs else 0
        hidden_channels = 64
        for k, v in state_dict.items():
            if re.match(r"convs\.0\.(lin\.weight|weight)", k):
                hidden_channels = v.shape[0]
                break
        try:
            gnn = NodeLevelGNN(
                in_channels=self.IN_CHANNELS, hidden_channels=hidden_channels, num_layers=num_layers
            )
            gnn.load_state_dict(state_dict, strict=False)
            self.gnn_model = gnn.to(self.device).eval()
            self.fc_layers = None
            print("Loaded GCN-style priority model.")
            return True
        except Exception:  # noqa: BLE001 - fall through to MLP attempt
            return False
    ...
    def score_nodes(self, node_features: np.ndarray, edge_index: Optional[np.ndarray] = None) -> np.ndarray:
        """Score a ``[num_nodes, 7]`` feature matrix, returning ``[num_nodes]``.

        ``edge_index`` (shape ``[2, E]``) lets a GCN checkpoint see the actual
        object-relation graph, matching what it saw during training. If
        omitted, a GCN falls back to self-loops only (no cross-object
        message passing) — the same behaviour as before this parameter
        existed. Ignored for MLP-format checkpoints, which never use edges.
        """
        ...
        with torch.no_grad():
            if self.gnn_model is not None:
                ...
                scores = self.gnn_model(x, edges_t).cpu().numpy()
            else:
